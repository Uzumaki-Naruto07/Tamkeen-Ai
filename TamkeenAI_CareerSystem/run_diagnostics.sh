...
echo "===== TamkeenAI Diagnostic Tools ====="
...
echo "Installing diagnostic requirements..."
pip install flask flask-cors || { echo "Failed to install packages"; exit 1; }

# Start the diagnostic backend in the background
echo "Starting diagnostic backend server on port 5050..."
python debug_backend.py &
BACKEND_PID=$!

# Create a minimal test page for frontend diagnostics
echo "Creating frontend diagnostic page..."
...
# Open diagnostic page in browser if possible
if command -v python3 &> /dev/null; then
    echo "Starting simple HTTP server for diagnostics page..."
    echo "Open http://localhost:8000/diagnostic.html in your browser"
    python3 -m http.server &
    HTTP_PID=$!
    
    # Try to open browser automatically
    if command -v xdg-open &> /dev/null; then
        xdg-open http://localhost:8000/diagnostic.html
    elif command -v open &> /dev/null; then
        open http://localhost:8000/diagnostic.html
    elif command -v start &> /dev/null; then
        start http://localhost:8000/diagnostic.html
    else
        echo "Please open http://localhost:8000/diagnostic.html in your browser"
    fi
fi

echo "Diagnostic tools started. Press Ctrl+C to stop."

# Wait for user to stop
trap "echo 'Stopping diagnostic tools...'; kill $BACKEND_PID 2>/dev/null; kill $HTTP_PID 2>/dev/null" EXIT
wait 